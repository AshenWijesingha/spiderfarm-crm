...
from spiderfarm.models import ZoneExtension, SpiderJob
...
class HomeView(SalesAccessRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if "ADMIN" in self.request.user.role or "MANAGER" in self.request.user.role or self.request.user.is_superuser:
            pass
        else:
            leads = leads.filter(
        ...
        # get tlds for Import Domain collapsable
        tlds = ZoneExtension.objects.all()
        context["tlds"] = tlds

        # current month lead stats
        now = datetime.datetime.now()
        _year = int(now.strftime("%Y"))
        _month = int(now.strftime("%m"))
        _month_name = now.strftime("%B")

        days_in_month = calendar.monthrange(_year, _month)
        _day_count = days_in_month[1]

        _monthly_leads = Lead.objects.filter(domain__ssl_expire__year=_year, domain__ssl_expire__month=_month)
        _total_leads = _monthly_leads.exclude(domain__ssl_expire__isnull=False)

        _daily_avg = int(_total_leads.count() / _day_count)

        context['month_name'] = _month_name
        context['daily_avg_leads'] = _daily_avg
        context['monthly_total_leads'] = _total_leads.count() 

        # query for running/pending Zone Import SSL spiderjobs
        running_zone_imports = SpiderJob.objects.filter(spider_type='IMPORT',
                                                        crawl_type='ZONE',
                                                        data_type='SSL',
                                                        job_status__in=['RUNNING','PENDING'])

        if running_zone_imports.count() > 0:
            # -- running SSL Zone Import, get the task id
            id_list = []
            for job in running_zone_imports:
                id_list.append(job.celery_id)
            # get unique items
            _set = set(id_list)
            uni_list = list(_set)
            
            if len(uni_list) > 1:
                print('-- error in SpiderJob Import queue, stale jobs present --')
                context['ssl_zone_import_error'] = 'error: stale jobs in SpiderJob queue --'
            else:
                context['ssl_zone_import_id'] = uni_list[0]
                print('-- running ssl zone import job: %s --' % uni_list[0])

        # throw in .com zonefrag counts
        context['pending_com_zones'] = tlds[0].count_zonefrags()
        context['imported_com_zones'] = tlds[0].count_zonefrags(imported=True)
...
class CreateUserView(AdminRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        print('-- inside CreateUserView.form_valid --')
...
class UpdateUserView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        # easy password changing if Admin
        if form.cleaned_data.get("password"):
            user.set_password(form.cleaned_data.get("password"))
        # -------------------------------
